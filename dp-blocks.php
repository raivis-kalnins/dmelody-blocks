...
define('DPBLOCKS_ROOT_PATH', plugin_dir_path(__FILE__));
define('DPBLOCKS_ROOT_URL', plugin_dir_url(__FILE__));

/**
 * Register and enqueue Gutenberg Blocks
 */
function register_dp_blocks() {
    $blocksPath = DPBLOCKS_ROOT_PATH . 'build/blocks/';
...
add_action('init', 'register_dp_blocks');
...
function wpdocs_enqueue_custom_public_style() {
	...
	$asset_path = DPBLOCKS_ROOT_PATH . $filepath . '.asset.php';
	$asset_file = file_exists( $asset_path ) ? include $asset_path : array( 'dependencies' => array(), 'version' => '1.0', );
	$script_url = DPBLOCKS_ROOT_URL . 'build/style/public.js';
	$style_url = DPBLOCKS_ROOT_URL . 'build/style/public.scss.css';
	$v = $asset_file['version'];
	wp_enqueue_style( 'public-dpblocks-style', $style_url, array(), $v, 'all' );
	//wp_enqueue_script( 'js_bootstrap', '//cdnjs.cloudflare.com/ajax/libs/bootstrap/5.2.0/js/bootstrap.min.js', ['jquery'], '', 'all' );
	wp_enqueue_script( 'public-dpblocks-style', $script_url, array(), $v, 'all' );
}
...
function wpdocs_enqueue_custom_admin_style() {
	...
	$asset_path = DPBLOCKS_ROOT_PATH . $filepath . '.asset.php';
	$asset_file = file_exists( $asset_path ) ? include $asset_path : array( 'dependencies' => array(), 'version' => '1.0', );
	$script_url = DPBLOCKS_ROOT_URL . 'build/style/admin.js';
	$style_url = DPBLOCKS_ROOT_URL . 'build/style/admin.scss.css';
	$v = $asset_file['version'];
	wp_enqueue_script( 'admin-dpblocks-style', $script_url, array(), $v, 'all' );
	wp_enqueue_style( 'admin-dpblocks-style', $style_url, array(), $v, 'all' );
}
