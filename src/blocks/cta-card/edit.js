...
const BlockEdit = (props) => {
	...
	return (
		<Fragment>
			<InspectorControls>
				<PanelBody title={__('Block Options', 'dp')} initialOpen={ true } >
					<div className="editor-post-featured-image">
							...
							<MediaUpload
								...
								render={({open}) => (
									<Button
										...
										onClick={open}
									>
										{attributes.mediaId == 0 && __('Choose an image', 'dp')}
						...
						{attributes.mediaId != 0 &&
							<MediaUploadCheck>
								<MediaUpload
									title={__('Replace image', 'dp')}
									value={attributes.mediaId}
									onSelect={onSelectMedia}
									allowedTypes={['image']}
									render={({open}) => (
										<Button onClick={open} isSecondary>{__('Replace image', 'dp')}</Button>
									)}
								/>
							</MediaUploadCheck>
						}
						{attributes.mediaId != 0 &&
							<MediaUploadCheck>
								<Button onClick={removeMedia} isLink isDestructive>{__('Remove image', 'dp')}</Button>
							</MediaUploadCheck>
						}
						<hr />
						<TextControl
